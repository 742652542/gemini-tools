...
def _build_image_files(image_list: list[bytes], mode: str) -> list[tuple[str, tuple[str, bytes, str]]]:
    files: list[tuple[str, tuple[str, bytes, str]]] = []
    for idx, image_binary in enumerate(image_list):
        image_ext = _get_image_extension(image_binary)
        image_mime = _get_image_mime(image_binary)
        field_name = "image[]" if mode == "bracket" else "image"
        files.append((field_name, (f"image_{idx}{image_ext}", image_binary, image_mime)))
    return files


def _image_upload_modes(image_count: int) -> list[str]:
    if image_count > 1:
        return ["bracket"]
    return ["single"]
...
def _run_generate_image(task_id: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    image_list = _normalize_images_bytes(payload.get("image"))
        ...
        try:
            with httpx.Client(timeout=UPSTREAM_TIMEOUT) as client:
                if image_list:
                    ...
                    mode_errors = []
                    for mode in _image_upload_modes(len(image_list)):
                        files = _build_image_files(image_list, mode)
                        resp = client.post(f"{UPSTREAM_BASE_URL}/images/edits", data=data, files=files, headers=headers)
                        try:
                            body = resp.json()
                        except Exception:
                            body = {"raw_text": resp.text}
                        _log_upstream_response(resp, body, "images.edits", mode)
